...
def test_create_new_meme(create_meme_endpoint, token, data):
...
def test_get_meme_by_id(get_meme_endpoint, token, meme_id):
    ...
    get_meme_endpoint.check_that_status_is_200()
    get_meme_endpoint.check_meme_id_matches(meme_id)  # Можно раскомментировать позже
...
def test_put_meme(update_meme_endpoint, token, meme_id):
    ...
    update_meme_endpoint.check_response_text_is_correct("Updated Meme Text")
    update_meme_endpoint.check_response_url_is_correct("https://lenta.ru/articles/2025/05/14/mem-okak/")
    update_meme_endpoint.check_response_tags_are_correct(["updated", "new"])


@allure.story('Delete meme')
@allure.title('Delete meme by ID')
@allure.tag('delete')
def test_delete_meme(delete_meme_endpoint, create_meme_endpoint, token):
    payload = {
        "text": "Meme to delete",
        "url": "https://lenta.ru/articles/2025/05/14/mem-okak/",
        "tags": ["delete"],
